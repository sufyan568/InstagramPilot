...
class Bot extends Command
{
    ...
    public $sender = []; // Save sender info to avoid multipel requests
    ...
    public function handle()
    {
        ...
        try {
            ...
            if (is_null($loginResponse)) {
                ...
                $push->on('direct_v2_message', function (PushNotification $push) use ($instagram) {

                    if ($push->getPushCategory() == 'direct_v2_pending') {
                        $this->info('New pending request');
                        $this->acceptPendingRequests($instagram);
                    }

                });

                $rtc->on('thread-item-created', function ($threadId, $threadItemId, DirectThreadItem $threadItem) use (
                    $push,
                    $loop,
                    $rtc,
                    $account,
                    ...
                    // Get sender info
                    if (!array_key_exists($threadItem->getUserId(), $this->sender)) {

                        $this->info('Getting sender info: ' . $threadItem->getUserId());

                        // Send request
                        $sender = $instagram->people->getInfoById($threadItem->getUserId())->getUser();

                        $this->sender[$threadItem->getUserId()] = [
                    ...
                    if ($bot) {
                        ...
                        $messageText = $threadItem->getText();

                        // Save transcription
                        $this->transcript[$threadId][] = [
                            'sender'  => $this->sender[$threadItem->getUserId()]['username'],
                            'message' => $messageText,
                        ];
                        ...
                        if ($this->welcomeTextSent[$threadId] == false && !is_null($bot->welcome_text)) {
                            ...
                            // Prepare text
                            $replyText = $this->prepareReplyText($bot->welcome_text, $threadItem->getUserId());
                            ...
                            if ($dialogueQA) {
                                ...
                                if ($isMatchedQA) {
                                    ...
                                    switch ($dialogueQA['message_type']) {
                                        ...
                                        case config('pilot.MESSAGE_TYPE_TEXT'):
                                            ...
                                            // Prepare text
                                            $replyText = $this->prepareReplyText($dialogueQA['message']['text'], $threadItem->getUserId());
                                ...
                                if ($bot->unknown_text) {
                                    ...
                                    // Spintax rotate
                                    $replyText = $this->prepareReplyText($bot->unknown_text, $threadItem->getUserId());
    ...
    private function prepareReplyText($text = '', $sender_id = null)
    {
        // Spintax rotate
        $spintax = new Spintax();
        ...
        // Placeholders
        if (array_key_exists($sender_id, $this->sender)) {
            $text = str_replace(':username', $this->sender[$sender_id]['username'], $text);
            $text = str_replace(':fullname', $this->sender[$sender_id]['fullname'], $text);
        }
